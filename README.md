# 🎓 SecretCoder - E-Learning Website

SecretCoder is a responsive E-Learning web application developed using *PHP, MySQL, HTML, CSS, JavaScript, and Bootstrap*. The platform allows users to create an account, enroll in courses, make payments, manage purchases, and update their profiles through a simple and user-friendly interface.

---

## 🚀 Features

- 🔐 User Registration (Sign Up)
- 🔑 Secure Login
- 🔒 Forgot Password
- 📩 OTP Verification
- 🔄 Reset Password
- 👤 User Dashboard
- 📚 Browse Courses
- 💳 Course Payment
- 🛒 My Purchases
- ✏️ Edit Profile
- ⭐ Course Review & Rating
- 📞 Contact Form
- 📱 Responsive Design

---

## 🛠️ Tech Stack

*Frontend*
- HTML5
- CSS3
- Bootstrap 5
- JavaScript

*Backend*
- PHP

*Database*
- MySQL

*Server*
- XAMPP / Apache
## 📸 Screenshots

### Sign Up
![Sign Up](https://github.com/karishma24-Itsme/SecretCoder-E-Learning/blob/main/signup.png)

### Login
![Login](https://github.com/karishma24-Itsme/SecretCoder-E-Learning/blob/main/login.png)

### Forgot Password
![Forgot Password](https://github.com/karishma24-Itsme/SecretCoder-E-Learning/blob/main/forgot-password.png)

### OTP Verification
![OTP](https://github.com/karishma24-Itsme/SecretCoder-E-Learning/blob/main/otp.png)

### Reset Password
![Reset Password](https://github.com/karishma24-Itsme/SecretCoder-E-Learning/blob/main/reset-password.png)

### Dashboard
![Dashboard](https://github.com/karishma24-Itsme/SecretCoder-E-Learning/blob/main/dashboard.png)

### Courses
![Courses](https://github.com/karishma24-Itsme/SecretCoder-E-Learning/blob/main/courses.png)

### Payment
![Payment](https://github.com/karishma24-Itsme/SecretCoder-E-Learning/blob/main/payment.png)

### My Purchase
![My Purchase](https://github.com/karishma24-Itsme/SecretCoder-E-Learning/blob/main/mypurchase.png)

### Edit Profile
![Edit Profile](https://github.com/karishma24-Itsme/SecretCoder-E-Learning/blob/main/edit-profile.png)

### Review
![Review](https://github.com/karishma24-Itsme/SecretCoder-E-Learning/blob/main/review.png)

### Contact
![Contact](https://github.com/karishma24-Itsme/SecretCoder-E-Learning/blob/main/contact.png)
